...
- **Pending cross-refs** — `RelatedEntries` today surfaces `PEND`
  markers for referenced-but-unauthored entries. Inventory count
  requires a content audit, but the infrastructure is honest: these
  don't render as broken links, they render as "pending editorial"
